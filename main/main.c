...
static bool ssid_pswd_flag[5] = { false };  // Ensure correct size
...
static int device_write(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg){
    ...
    case API_TOKEN_UUID:
        ...
        if (ctxt == NULL || ctxt->om == NULL || ctxt->om->om_data == NULL) {
            ESP_LOGI(TAG, "BLE write received NULL API Token data");
            break; // Stop execution if data is invalid
        }

        // Ensure proper length
        len = (ctxt->om->om_len < sizeof(main_struct.apiToken) - 1) ? ctxt->om->om_len : sizeof(main_struct.apiToken) - 1;

        // Check if the input is too large for the buffer
        if (ctxt->om->om_len > sizeof(main_struct.apiToken) - 1) {
            ESP_LOGI(TAG, "API Token too large. Truncating.");
        }

        // Copy the data and null terminate it
        strncpy(main_struct.apiToken, (char *)ctxt->om->om_data, len);
        main_struct.apiToken[len] = '\0'; // Null terminate

        ESP_LOGI(TAG, "Received Device API Token: %s", main_struct.apiToken);  // Print the API token as a string
        break;
    ...
    // Check if all required data has been received before saving
    if (ssid_pswd_flag[0] && ssid_pswd_flag[1] && ssid_pswd_flag[2] && ssid_pswd_flag[3] && ssid_pswd_flag[4]){
